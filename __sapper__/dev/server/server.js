...
<main><h2 class="${"is-size-3"}">Tiger Team1</h2>
  ${members
	? `<table class="${"table is-striped"}"><thead><tr><th>Company</th>
      ...
      <tbody>${each(members, (member, i) => `<tr><th>${escape(member.company)}</th>
            ...
            <td>${escape((current[i].contributed - member.contributed) / member.contributed)}</td>
            <td>${escape((current[i].contributed - member.contributed) / current[i].contributed * 100)}%</td>
          </tr>`)}</tbody></table>`
...
const q$1 = faunadb.query;
const client$1 = new faunadb.Client({
  secret: 'fnADvsSZphACAJMZtbFfkj4b83lYv_oQQGQRCyce',
});
...
const writeData = (team) => {
  client$1
    .query(
      q$1.Replace(q$1.Ref(q$1.Collection('team'), '270057444654187008'), {
        data: {
          members: team,
        },
      })
